...
const LoginUser = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      router.push("/screens/User/Home"); // ajuste esse path conforme a tela principal do usuário
    } catch (error: any) {
    ...
    <LinearGradient colors={['#5d5d5d', '#777777']} style={{ flex: 1 }}>
      <View className="h-full w-full p-10 flex gap-10 justify-center rounded-3xl">
        ...
        <View className="flex items-center mt-4">
          <Text className="text-sm text-white">Ainda não tem uma conta?</Text>
          <TouchableOpacity onPress={() => router.push("/screens/User/Cadastro")}>
            <Text className="text-blue-300 font-medium">Cadastre-se agora</Text>
          </TouchableOpacity>
        </View>
      </View>
    </LinearGradient>
